      ...
      <xsl:choose>
        ...
        <xsl:when test="$memberType = 'method' and /bundle/assembly[ld:cmpnover(@assetId, current()/ancestor::assembly/@assetId)]/namespace//*/method[@assetId and ld:cmpnover(@assetId, current()/@assetId) and (preceding-sibling::method/@name = @name or following-sibling::method/@name = @name)]/@assetId">
          ...
          <div class="version-selector">
            ...
            <ul>
              <xsl:apply-templates select="/bundle/assembly[ld:cmpnover(@assetId, current()/ancestor::assembly/@assetId)]/namespace//*[@assetId and ld:cmpnover(@assetId, current()/@assetId) and (preceding-sibling::method/@name = @name or following-sibling::method/@name = @name)]/@assetId" mode="version"/>
            </ul>
          </div>
        </xsl:when>
        <xsl:when test="$memberType = 'operator' and /bundle/assembly[ld:cmpnover(@assetId, current()/ancestor::assembly/@assetId)]/namespace//*/operator[@assetId and ld:cmpnover(@assetId, current()/@assetId) and (preceding-sibling::operator/@name = @name or following-sibling::operator/@name = @name)]/@assetId">
          <xsl:text>&#160;|&#160;</xsl:text>
          <div class="version-selector">
            <a href="javascript:void(0);">
              <xsl:text>Other Versions</xsl:text>
            ...
            <ul>
              <xsl:apply-templates select="/bundle/assembly[ld:cmpnover(@assetId, current()/ancestor::assembly/@assetId)]/namespace//*[@assetId and ld:cmpnover(@assetId, current()/@assetId) and (preceding-sibling::operator/@name = @name or following-sibling::operator/@name = @name)]/@assetId" mode="version"/>
            </ul>
          </div>
        </xsl:when>
      </xsl:choose>
        ...
        <xsl:choose>
          <xsl:when test="@overrides">
            <xsl:if test="not(doc:summary)">
              ...
              <xsl:if test="not(//*[@assetId = current()/@overrides]/doc:summary)">
          ...
          <xsl:when test="@declaredAs">
            <xsl:if test="not(doc:summary)">
              ...
              <xsl:if test="not(//*[@assetId = current()/@declaredAs]/doc:summary)">
                <xsl:call-template name="missing"/>
              </xsl:if>
            </xsl:if>
            <xsl:text> (Inherited from </xsl:text>
            <a href="{ld:resolve(//*[@assetId = current()/@declaredAs]/parent::*[@assetId]/@assetId)}">
              <xsl:apply-templates select="//*[@assetId = current()/@declaredAs]/parent::*" mode="displayText"/>
            </a>
            <xsl:text>)</xsl:text>
          </xsl:when>
          <xsl:otherwise>
