      ...
      .forEach(function (token, i, tokens) {
        ...
        var set = this.tokenStore.expand(token).reduce(function (memo, key) {
              var pos = self.corpusTokens.indexOf(key),
                  idf = self.idf(key),
                  similarityBoost = 1,
                  set = new lunr.SortedSet; // if the expanded key is not an exact match to the token then
              ...
              if (pos > -1) queryArr[pos] = tf * idf * similarityBoost; // add all the documents that have this key into a set
